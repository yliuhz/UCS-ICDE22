...
int main(void)
{
    ...
    vector<vector<int>> qss = {qs[0][0], qs[1][0], qs[2][0], qs[3][0]}; //TIPS: 选取core_number=3的顶点
    ...
    for(int i = 0; i < files.size(); i++){
        ...
        vector<int> qss_of_this_file = qss[i];
        ...
        printf("%s: V=%d, E=%d\n", file.c_str(), (int)b.V, (int)b.E);

        for(int j = 0; j < qn; j++){
            int q = qss_of_this_file[j];

            auto Cqk = lo.global(q, k, G);
            auto G_pruned = lo.extractSubgraph(Cqk, G);
            G_pruned = lo.rearrange(G_pruned, q, Cqk);
            auto Edges_pruned = lo.table2Edges(G_pruned);
            auto dG_pruned = b.toDeter(G_pruned);

            vector<int> C;
            for(auto v : Cqk){
                if(pkc.degGeK(G, v, k) >= theta){
                    C.emplace_back(v);
                }
            }

            printf("G size=%d, Cqk size=%d, Edges size=%d, Edges_pruned size=%d, C size=%d\n",
                   (int)G.size(), (int)G_pruned.size(), (int)Edges.size(), (int)Edges_pruned.size(),
                   (int)C.size());
            ...
            START
            mcs_with_prune_log(G_pruned, Edges_pruned, C, q, k, theta, Pr_v, file, 1000);
            END("mcs")
        }
    }
