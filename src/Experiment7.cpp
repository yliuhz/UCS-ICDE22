...
// TIPS: 测时间，剪枝I,II后剩下的候选集大小, k变, \theta=0.8
// 1520
//
...
//    vector<int> ks = {10, 30, 10, 134};
    ...
    for(int i = 0; i < files.size(); i++){
        ...
        string outfile = "output/"+file+"_Experiment7(k2sizes).txt";
        ...
        unordered_map<int,vector<vector<decltype(G.size())>>> k2sizes;
        ...
        for(int l = 1; l < ks_of_this_file.size(); l++){
            ...
            for(int ll = 0; ll < qs_of_this_k.size(); ll++){
                ...
                k2sizes[k].emplace_back(vector<decltype(G.size())>{G.size(),G_prune_i.size(),C.size()});
            }
        }
        ...
        for(auto [k, sizes_of_qs] : k2sizes){
            out<<"k="<<k<<",qn="<<sizes_of_qs.size()<<endl;
