...
// TIPS: 测时间，剪枝I,II后剩下的候选集大小, k=35, \theta变
// 1520
//
...
//    vector<int> ks = {10, 30, 10, 134};
    ...
    for(int i = 0; i < files.size(); i++){
        ...
        string outfile = "output/"+file+"_Experiment8(theta2sizes).txt";
        ...
        unordered_map<double,vector<vector<decltype(G.size())>>> theta2sizes;
        ...
        for(int l = 7; l < 8; l++){
            ...
            for(int ll = 0; ll < qs_of_this_k.size(); ll++){
                ...
                for(auto lll = 0; lll < thetas.size(); lll++){
                    double theta = thetas[lll];

                    printf("%s: %d/%d theta, %d/%d q\n", file.c_str(),
                    ...
                    theta2sizes[theta].emplace_back(vector<decltype(G.size())>{G.size(),G_prune_i.size(),C.size()});
                }
            }
        }
        ...
        for(auto [theta, sizes_of_qs] : theta2sizes){
            out<<"theta="<<theta<<",qn="<<sizes_of_qs.size()<<endl;
            for(auto sizes : sizes_of_qs){
                for(auto size : sizes){
                    out<<size<<" ";
                }
                out<<endl;
