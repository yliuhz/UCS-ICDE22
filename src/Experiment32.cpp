...
//    vector<int> ks = {10, 30, 10, 134};
    ...
    for(int i = 0; i < files.size(); i++){
        ...
        unordered_map<int,vector<double>> k2time_pos;
...
//                ans = mcs_with_prune(G_prune_i, Edges_prune_i, C, q, k, theta, Pr_v, 1000);
                ans = EXACT_POS(G_prune_i, Edges_prune_i, q, k, theta, Pr_v);
                end = clock();
                time = (double)(end-start)/CLOCKS_PER_SEC;
                k2time_pos[k].emplace_back(time);
                printf("Time of EXACT-POS: %.2f s\n", time);
            }
        }
        ...
        out<<"MCS"<<endl;
        for(auto [k, time_of_qs] : k2time_pos){
