    ...
    for(int i = 0; i < 1; i++){ // TIPS: 只测email
        ...
        for(int l = 7; l < 8; l++){
            ...
            for(int ll = 0; ll < 1; ll++){ // TIPS: 只用一个点
                ...
                start = clock();
                ans = EXACT_POS(G, Edges, C0, q, k, theta, Pr_v);
                end = clock();
                time = (double)(end-start)/CLOCKS_PER_SEC;
                exact_no_pruning.emplace_back(time);
                printf("Time of exact: %.2f s\n", time);
            }
        }
        ...
        avgtime = 0.0;
        out<<"exact_no_pruning";
        for(int j = 0; j < exact_no_pruning.size(); j++){
            if(j % 10 == 0) out<<endl;
            out<<exact_no_pruning[j]<<" ";
            avgtime += exact_no_pruning[j];
        }
        out<<endl;
        avgtime /= 1;
