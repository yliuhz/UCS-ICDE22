...
// TIPS: 测方差，只测email, k=3, \theta=0.8, N变化
// 1520
//
...
//    vector<int> Ns = {500, 1000, 1500, 2000};
    vector<int> Ns = {2500, 3000, 3500, 4000};
    ...
    for(int i = 0; i < 1; i++){ // TIPS: 从email开始
        ...
        for(int l = 0; l < 1; l++) { //
            ...
            unordered_map<int,vector<double>> N2vars_mcs, N2vars_ss_vl, N2vars_ss_sc,
            N2vars_ls, N2vars_lssa_vl, N2vars_lssa_sc;
            ...
            for (int ll = 0; ll < qn; ll++) {
                ...
                for(int j = 0; j < Ns.size(); j++){
                    ...
                    for (int lll = 0; lll < n0; lll++) {
                        printf("%s:  1/1 k, %d/%d q, %d/%d N, %d/%d loop, mcs\n",
                               file.c_str(),
                               ll + 1, qn,
                               j+1, (int)Ns.size(),
                               lll + 1, n0);
                        ...
                        START
                        mcs_with_prune(G_prune_i, Edges_prune_i, C, q, k, theta, Pr_v, N);
                        END("MCS")

                        if (Pr_v_avg.empty()) Pr_v_avg = Pr_v;
                        else {
                            for (int llll = 0; llll < Pr_v.size(); llll++) {
                                Pr_v_avg[llll] += Pr_v[llll];
                            }
                        }
                        Prs.emplace_back(Pr_v);
                    }

                    for (int lll = 0; lll < Pr_v_avg.size(); lll++) {
                        Pr_v_avg[lll] /= n0;
                    }

                    var = 0.0;
                    for (int lll = 0; lll < n0; lll++) {
                        var += norm2(Prs[lll], Pr_v_avg);
                    }
                    var /= n0 - 1;

                    N2vars_mcs[N].emplace_back(var);

                    cout<<"var_mcs="<<var<<endl;

                    // TIPS: SS-VL
                    Pr_v_avg.clear();
                    Prs.clear();
                    for (int lll = 0; lll < n0; lll++) {
                        printf("%s:  1/1 k, %d/%d q, %d/%d N, %d/%d loop, ss-vl\n",
                               file.c_str(),
                               ll + 1, qn,
                               j+1, (int)Ns.size(),
                               lll + 1, n0);
                        ...
                        START
                        ss_vl(G_prune_i, Edges_prune_i, C, q, k, theta, Pr_v, N);
                        END("SS-VL")
                    ...
                    N2vars_ss_vl[N].emplace_back(var);

                    cout<<"var_ss_sl="<<var<<endl;

                    // TIPS: SS-SC
                    Prs.clear();
                    Pr_v_avg.clear();
                    for (int lll = 0; lll < n0; lll++) {
                        printf("%s:  1/1 k, %d/%d q, %d/%d N, %d/%d loop, ss-sc\n",
                               file.c_str(),
                               ll + 1, qn,
                               j+1, (int)Ns.size(),
                               lll + 1, n0);
                        ...
                        START
                        ss_sc(G_prune_i, Edges_prune_i, dG_prune_i, C, q, k, theta, Pr_v, N);
                        END("SS-SC")
                    ...
                    N2vars_ss_sc[N].emplace_back(var);

                    cout<<"var_ss_sc="<<var<<endl;

                    // TIPS: LS
                    Pr_v_avg.clear();
                    Prs.clear();
                    for (int lll = 0; lll < n0; lll++) {
                        printf("%s:  1/1 k, %d/%d q, %d/%d N, %d/%d loop, ls\n",
                               file.c_str(),
                               ll + 1, qn,
                               j+1, (int)Ns.size(),
                               lll + 1, n0);
                        ...
                        START
                        ls(G_prune_i, Edges_prune_i, C, q, k, theta, Pr_v, N);
                        END("LS")
                    ...
                    N2vars_ls[N].emplace_back(var);

                    cout<<"var_ls="<<var<<endl;

                    // TIPS: LSSA-VL
                    Pr_v_avg.clear();
                    Prs.clear();
                    for (int lll = 0; lll < n0; lll++) {
                        printf("%s:  1/1 k, %d/%d q, %d/%d N, %d/%d loop, lssa-vl\n",
                               file.c_str(),
                               ll + 1, qn,
                               j+1, (int)Ns.size(),
                               lll + 1, n0);
                        ...
                        START
                        lssa_vl(G_prune_i, Edges_prune_i, C, q, k, theta, Pr_v, N);
                        END("LSSA-VL")
                    ...
                    N2vars_lssa_vl[N].emplace_back(var);

                    cout<<"var_lssa_vl="<<var<<endl;

                    // TIPS: LSSA-SC
                    Pr_v_avg.clear();
                    Prs.clear();
                    for (int lll = 0; lll < n0; lll++) {
                        printf("%s:  1/1 k, %d/%d q, %d/%d N, %d/%d loop, lssa-sc\n",
                               file.c_str(),
                               ll + 1, qn,
                               j+1, (int)Ns.size(),
                               lll + 1, n0);
                        ...
                        START
                        lssa_sc(G_prune_i, Edges_prune_i, dG_prune_i, C, q, k, theta, Pr_v, N);
                        END("LSSA-SC")
                    ...
                    N2vars_lssa_sc[N].emplace_back(var);

                    cout<<"var_lssa_sc="<<var<<endl;
                }
            }
            ...
            for(auto [N, vars_of_qs] : N2vars_mcs){
                var_avg = 0.0;
                out<<"N="<<N;
                for (int j = 0; j < qn; j++) {
                    if (j % 10 == 0) out << endl;
                    out << vars_of_qs[j] << " ";
                    var_avg += vars_of_qs[j];
                }
                out << endl;
            ...
            for(auto [N, vars_of_qs] : N2vars_ss_vl){
                var_avg = 0.0;
                out<<"N="<<N;
                for (int j = 0; j < qn; j++) {
                    if (j % 10 == 0) out << endl;
            ...
            for(auto [N, vars_of_qs] : N2vars_ss_sc){
                var_avg = 0.0;
                out<<"N="<<N;
                for (int j = 0; j < qn; j++) {
                    if (j % 10 == 0) out << endl;
            ...
            for(auto [N, vars_of_qs] : N2vars_ls){
                var_avg = 0.0;
                out<<"N="<<N;
                for (int j = 0; j < qn; j++) {
                    if (j % 10 == 0) out << endl;
            ...
            for(auto [N, vars_of_qs] : N2vars_lssa_vl){
                var_avg = 0.0;
                out<<"N="<<N;
                for (int j = 0; j < qn; j++) {
                    if (j % 10 == 0) out << endl;
            ...
            for(auto [N, vars_of_qs] : N2vars_lssa_sc){
                var_avg = 0.0;
                out<<"N="<<N;
                for (int j = 0; j < qn; j++) {
                    if (j % 10 == 0) out << endl;
