...
int main(void)
{
    ...
    for(int i = 0; i < 1; i++){ // TIPS: 从email开始
        ...
        for(int l = 0; l < 1; l++) { //
            ...
            vector<double> k0s = {0.25*k, 0.5*k, 1.0*k, 2.0*k, 3.0*k, 4.0*k};

            auto qs_of_this_k = qs_of_this_file[0]; // TIPS: 选择core_number=3的查询点

            unordered_map<double,vector<double>> k02vars_ss_vl, k02vars_lssa_vl;
            for (int ll = 0; ll < qn; ll++) {
                ...
                for(int j = 0; j < k0s.size(); j++){
                    ...
                    for (int lll = 0; lll < n0; lll++) {
                        printf("%s:  1/1 k, %d/%d q, %d/%d k0, %d/%d loop, ss-vl\n",
                               file.c_str(),
                               ll + 1, qn,
                               j+1, (int)k0s.size(),
                               lll + 1, n0);
                        ...
                        START
                        ss_vl(G_prune_i, Edges_prune_i, C, q, k, theta, Pr_v, k0);
                        END("SS-VL")
                    ...
                    k02vars_ss_vl[k0].emplace_back(var);
                    ...
                    Prs.clear();
                    for (int lll = 0; lll < n0; lll++) {
                        printf("%s:  1/1 k, %d/%d q, %d/%d k0, %d/%d loop, lssa-vl\n",
                               file.c_str(),
                               ll + 1, qn,
                               j+1, (int)k0s.size(),
                               lll + 1, n0);
                        ...
                        START
                        lssa_vl(G_prune_i, Edges_prune_i, C, q, k, theta, Pr_v, k0);
                        END("LSSA-VL")
                    ...
                    k02vars_lssa_vl[k0].emplace_back(var);
            ...
            for(auto [k0, vars_of_qs] : k02vars_ss_vl){
                var_avg = 0.0;
                out<<"k0="<<k0;
                for (int j = 0; j < qn; j++) {
                    if (j % 10 == 0) out << endl;
            ...
            for(auto [k0, vars_of_qs] : k02vars_lssa_vl){
                var_avg = 0.0;
                out<<"k0="<<k0;
                for (int j = 0; j < qn; j++) {
                    if (j % 10 == 0) out << endl;
