...
//    vector<int> ks = {10, 30, 10, 134};
    ...
    for(int i = 0; i < files.size(); i++){
        ...
        string outfile = "output/"+file+"_Experiment19.txt";
        ...
        vector<double> mcs_no_pruning, mcs_with_pruning_i, mcs_with_pruning_i_ii;

        printf("%s: Start evaluating ...\n", file.c_str());

        for(int l = 7; l < 8; l++){
            int k = ks_of_this_file[l]; // TIPS: k从35开始
            ...
            for(int ll = 0; ll < qs_of_this_k.size(); ll++){
                ...
                vector<int> C0(b.V);
                iota(C0.begin(), C0.end(), 0);
                start = clock();
                ans = mcs_with_prune(G, Edges, C0, q, k, theta, Pr_v, 1000);
                end = clock();
                time = (double)(end-start)/CLOCKS_PER_SEC;
                mcs_no_pruning.emplace_back(time);
                printf("Time of MCS: %.2f s\n", time);

                start = clock();
                ans = mcs_with_prune(G_prune_i, Edges_prune_i, Cqk, q, k, theta, Pr_v, 1000);
                end = clock();
                time = (double)(end-start)/CLOCKS_PER_SEC;
                mcs_with_pruning_i.emplace_back(time);
                printf("Time of MCS+pruning i: %.2f s\n", time);

                start = clock();
                ans = mcs_with_prune(G_prune_i, Edges_prune_i, C, q, k, theta, Pr_v, 1000);
                end = clock();
                time = (double)(end-start)/CLOCKS_PER_SEC;
                mcs_with_pruning_i_ii.emplace_back(time);
                printf("Time of MCS+pruning i,ii: %.2f s\n", time);
            }
        }
        ...
        avgtime = 0.0;
        out<<"MCS_no_pruning";
        for(int j = 0; j < mcs_no_pruning.size(); j++){
            if(j % 10 == 0) out<<endl;
            out<<mcs_no_pruning[j]<<" ";
            avgtime += mcs_no_pruning[j];
        }
        out<<endl;
        avgtime /= 20;
        out<<"avgtime="<<avgtime<<endl;

        avgtime = 0.0;
        out<<"MCS_with_pruning_i";
        for(int j = 0; j < mcs_with_pruning_i.size(); j++){
            if(j % 10 == 0) out<<endl;
            out<<mcs_with_pruning_i[j]<<" ";
            avgtime += mcs_with_pruning_i[j];
        }
        out<<endl;
        avgtime /= 20;
        out<<"avgtime="<<avgtime<<endl;
        ...
        avgtime = 0.0;
        out<<"MCS_with_pruning_i_ii";
        for(int j = 0; j < mcs_with_pruning_i_ii.size(); j++){
            if(j % 10 == 0) out<<endl;
            out<<mcs_with_pruning_i_ii[j]<<" ";
            avgtime += mcs_with_pruning_i_ii[j];
        }
        out<<endl;
        avgtime /= 20;
        out<<"avgtime="<<avgtime<<endl;
