...
//    vector<int> ks = {10, 30, 10, 134};
    ...
    for(int i = 0; i < files.size(); i++){
        ...
        string outfile = "output/"+file+"_Experiment32(k2time).txt";
        ...
        unordered_map<int,vector<double>> k2time_enum;
...
//                       (int)C.size());
                ...
                printf("G size=%d, G_pruned size=%d, Edges size=%d, Edges_pruned size=%d\n",
                       (int)G.size(), (int)G_prune_i.size(),
                       (int)Edges.size(), (int)Edges_prune_i.size());
...
//                ans = mcs_with_prune(G_prune_i, Edges_prune_i, C, q, k, theta, Pr_v, 1000);
                ans = EXACT_ENUM(G_prune_i, Edges_prune_i, q, k, theta, Pr_v);
                end = clock();
                time = (double)(end-start)/CLOCKS_PER_SEC;
                k2time_enum[k].emplace_back(time);
                printf("Time of EXACT-ENUM: %.2f s\n", time);
            }
        }
        ...
        out<<"MCS"<<endl;
        for(auto [k, time_of_qs] : k2time_enum){
