...
int main(void)
{
    ...
    vector<int> Ns = {250, 500, 750, 1000, 1250, 1500};
    ...
    for(int i = 0; i < 1; i++){ // TIPS: 从email开始
        ...
        for(int l = 1; l < 2; l++) { //
            int k = ks_of_this_file[l]; // TIPS: k=5

            auto qs_of_this_k = qs_of_this_file[8]; // TIPS: 选择core_number=40的查询点

            unordered_map<int,vector<double>> N2time_mcs, N2time_ss_sc,
                    N2time_ls, N2time_lssa_sc;
            ...
            for (int ll = 0; ll < qn; ll++) {
                ...
                for(int j = 0; j < Ns.size(); j++){
                    ...
                    for (int lll = 0; lll < n0; lll++) {
                        ...
                        N2time_mcs[N].emplace_back(time);
                    }
                    ...
                    for (int lll = 0; lll < n0; lll++) {
                        ...
                        start = clock();
                        ss_sc(G_prune_i, Edges_prune_i, C, q, k, theta, Pr_v, N);
                        end = clock();
                        time = (double) (end - start) / CLOCKS_PER_SEC;

                        N2time_ss_sc[N].emplace_back(time);
                    }
                    ...
                    for (int lll = 0; lll < n0; lll++) {
                        ...
                        start = clock();
                        lssa_sc(G_prune_i, Edges_prune_i, C, q, k, theta, Pr_v, N);
                        end = clock();
                        time = (double) (end - start) / CLOCKS_PER_SEC;

                        N2time_lssa_sc[N].emplace_back(time);
                    }
                }
            }
            ...
            out << "k=" << k<<endl;
            for(auto [N, time_of_qs] : N2time_mcs){
                out<<"N="<<N;
                for (int j = 0; j < qn; j++) {
                    if (j % 10 == 0) out << endl;
                    out << time_of_qs[j] << " ";
            ...
            out << "k=" << k << endl;
            for(auto [N, time_of_qs] : N2time_ss_sc){
                out<<"N="<<N;
                for (int j = 0; j < qn; j++) {
                    if (j % 10 == 0) out << endl;
                    out << time_of_qs[j] << " ";
            ...
            out << "k=" << k << endl;
            for(auto [N, time_of_qs] : N2time_ls){
                out<<"N="<<N;
                for (int j = 0; j < qn; j++) {
                    if (j % 10 == 0) out << endl;
                    out << time_of_qs[j] << " ";
            ...
            out << "k=" << k << endl;
            for(auto [N, time_of_qs] : N2time_lssa_sc){
                out<<"N="<<N;
                for (int j = 0; j < qn; j++) {
                    if (j % 10 == 0) out << endl;
                    out << time_of_qs[j] << " ";
