...
//    vector<int> ks = {10, 30, 10, 134};
    ...
    vector<double> thetas = {0.1, 0.3, 0.5, 0.7, 0.9};
    ...
    for(int i = 3; i < 4; i++){ // TIPS: 只测Biomine
        ...
        string outfile = "output/"+file+"_Experiment30.txt";
        ...
        unordered_map<double,vector<double>> theta2time;
        ...
        for(int l = 7; l < 8; l ++){
            ...
            for(int lll = 0; lll < thetas.size(); lll++){
                ...
                for(int ll = 0; ll < qs_of_this_k.size(); ll++){
                    ...
                    start = clock();
                    ans = baseline(G, Edges, q, k, theta, Pr_v, 1000);
                    end = clock();
                    time = (double)(end-start)/CLOCKS_PER_SEC;
                    theta2time[theta].emplace_back(time);
                    printf("theta=%.1f, %d/%d q, Time of MCS: %.2f s\n", theta, ll+1, (int)qs_of_this_k.size(), time);
                }
            }
        }
        ...
        out<<"MCS_no_pruning"<<endl;
        for(auto [theta,ts] : theta2time){
            out<<"theta="<<theta;
            avgtime = 0.0;
