...
//    vector<int> ks = {10, 30, 10, 134};
    ...
    for(int i = 0; i < 1; i++){ // TIPS: 只测email
        ...
        vector<double> exact_no_pruning, exact_with_pruning_i, exact_with_pruning_i_ii;
        ...
        for(int l = 7; l < 8; l++){
            ...
            for(int ll = 0; ll < 1; ll++){ // TIPS: 只用一个点
                ...
                start = clock();
                ans = EXACT_POS(G_prune_i, Edges_prune_i, C, q, k, theta, Pr_v);
                end = clock();
                time = (double)(end-start)/CLOCKS_PER_SEC;
                exact_with_pruning_i_ii.emplace_back(time);
                printf("Time of exact+pruning i,ii: %.2f s\n", time);
            }
        }
        ...
        avgtime = 0.0;
        out<<"exact_with_pruning_i_ii";
        for(int j = 0; j < exact_with_pruning_i_ii.size(); j++){
            if(j % 10 == 0) out<<endl;
            out<<exact_with_pruning_i_ii[j]<<" ";
            avgtime += exact_with_pruning_i_ii[j];
        }
        out<<endl;
        avgtime /= 1;
