...
// TIPS: 测方差，只测biomine, k=3, theta=0.8
// 1520
//
...
int main(void)
{
    ...
    vector<vector<vector<int>>> qs = {
            ...
            { // Flickr
                    { // k = 3
                            ...
                            2943, 2981, 3039, 3043, 3057, 3070, 3087, 3162, 3243, 3244,
                            3247, 3276, 3277, 3281, 3282, 3905, 4119, 4219, 4221, 4488,
                            5051, 5052, 5053, 5060, 5063, 5068, 5083, 5204, 5592, 5600,
                    },
                    { // k = 5
                    ...
                    { // k = 40
                            ...
                            5031, 5225, 5317, 5396, 5746, 5950, 6304, 6381, 6994, 7030,
                            7378, 7497, 7555, 7577, 7799, 8130, 8137, 8152, 8224, 8437,
                            8606, 9258, 9376, 9487, 9509, 12739, 12954, 12978, 13247, 13299,
                    },
                    { // k = 45
    ...
    for(int i = 3; i < 4; i++){ // TIPS: 只测biomine
        ...
        string outfile = "output/Experiment3_"+file;
        ...
        for(int l = 1; l < 2; l++) { //
            ...
            for (int ll = qn-1; ll >= 0; ll--) {
                int q = qs_of_this_k[ll];
                string out_pr_q = "output/mcs_pr_of_q_"+to_string(q)+".txt";
                ofstream out_stream;

                out<<"k="<<k<<" q="<<q<<endl;
                cout<<"k="<<k<<" "<<qn-ll<<"/"<<qn<<" q="<<q<<endl;
                ...
                cout<<"Candidate set size: "<<G_prune_i.size()<<endl;

                // TIPS: LSSA with prune
                for (int lll = 0; lll < n0; lll++) {
                    out_stream.open(out_pr_q, ios::app);
                    assert(!out_stream.fail());
                    cout<<lll+1<<"/"<<n0<<" loops"<<endl;
                    vector<double> Pr_v;
                    START
                    lssa_sc_f(G_prune_i, Edges_prune_i, Cqk, q, k, theta, Pr_v, N);
                    END("lssa")
                    out_stream<<"lll="<<lll<<endl;
                    for(auto p : Pr_v){
                        out_stream<<p<<" ";
                    }
                    out_stream<<endl;
