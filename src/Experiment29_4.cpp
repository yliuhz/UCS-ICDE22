...
//    vector<int> ks = {10, 30, 10, 134};
    ...
    for(int i = 3; i < 4; i++){ // TIPS: 只测Biomine
        ...
        string outfile = "output/"+file+"_Experiment29.txt";
        ...
        unordered_map<int,vector<double>> k2time;

        printf("%s: Start evaluating ...\n", file.c_str());

        for(int l = 7; l >= 0; l -= 2){
            int k = ks_of_this_file[l]; // TIPS: k={35,25,15,5}
            ...
            for(int ll = 0; ll < qs_of_this_k.size(); ll++){
                ...
                k2time[k].emplace_back(time);
                printf("k=%d, %d/%d q, Time of MCS: %.2f s\n", k, ll+1, (int)qs_of_this_k.size(), time);
            }
        }
